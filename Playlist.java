...
public class Playlist {
    ...
    // Abstraction Function: play list songs
    /**AF(songs) = Function นี้มีเอาไว้เพื่อบอกลำดับของเพลงที่ song.(1),song.(2)...ไปเรื่อยๆโดยที่เพลงเเรกจะขึ้นเล่นบนlist play ก่อน */

    // TODO 2: เขียน Representation Invariant ตรงนี้ (4 ข้อ)
    // Representation Invariant: Rule of play list songs
    /** ต้องมีlisy playจริงๆ ที่ไม่ใช่ null, ไม่มีเพลงที่เป็น null,ไม่มีเพลงที่เป็นstring ว่าง, ชื่อเพลงห้ามซ้ำ,มีเพลงในlist ได้ไม่เกิน 100 = Max_Songs(100)*/

    // TODO 3: เขียน Safety from rep exposure ตรงนี้
    // Safety from rep exposure: เป็นวิธีการป้องกันการรั่วไหลของplay list songs
    /**มีการcopy obj ทั้งตอนสร้างเเละส่งplay list song */
    ...
    public Playlist(List<String> initial) {
        ...
        if (initial==null || initial.size() > MAX_SONGS) {
    ...
    public boolean add(String song) {
        ...
        if (songs.size() == MAX_SONGS || songs.contains(song)) {
            return false;
        }
        songs.add(song);
        checkRep();
